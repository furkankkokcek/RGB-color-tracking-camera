...
#define SSID_NAME  "AndroidA"
#define PASSKEY    "21427468"
...
char returnBuffer(int index){
	return ServerResponseBuffer[index];
}
//-------------------SearchStart -------------------
...
int ESP8266_MakeTCPConnection(char *IPaddress){
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CIPSTART=\"UDP\",\"%s\",4445\r\n", IPaddress);
...
int ESP8266_SendTCP(char* fetch){
  ...
  while(n&&(ServerResponseSearchFinished==0)){
    time = (25000*8)/91;  // 1msec, tuned at 80 MHz
    while(time){
      time--;
    }
    n--;
